...
- **Integración de sombra dinámica:** posición solar con pvlib + modelo de alturas de edificios del vuelo LiDAR disponible en el Geoportal de Madrid + proyección de sombra por tramo y hora. 
  - **Prerrequisito de datos:** Validar la resolución (puntos/m²) y la cobertura temporal (año de vuelo) del dataset LiDAR municipal para asegurar la precisión volumétrica y vigencia.
  - **Prerrequisito arquitectónico:** Precalcular la matriz sombra×hora×tramo offline para no bloquear el routing interactivo en tiempo real.
